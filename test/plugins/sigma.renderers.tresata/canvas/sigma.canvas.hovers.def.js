module.exports = function(sigma) {
  ...
  sigma.canvas.hovers.def = function(node, context, settings) {
    ...
    // Node:
    var nodeRenderer = sigma.canvas.nodes[node && node.type || null] || sigma.canvas.nodes.def;
    nodeRenderer(node, context, settings);

    // Display the label:
    if (node && node.label && typeof node.label === 'string') {
    ...
    function drawHoverBorder(alignment, context, fontSize, node) {
      var x = Math.round(node && node[prefix + 'x'] || 1),
          y = Math.round(node && node[prefix + 'y'] || 1),
          w = Math.round(
            context.measureText(node && node.label || '').width + size + 1.5 + fontSize / 3
          ),
          h = fontSize + 4,
